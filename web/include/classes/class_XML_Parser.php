...
if ( !function_exists('ini_size_to_bytes') OR (($current_memory_limit = ini_size_to_bytes(@ini_get('memory_limit'))) < MIN_LIMIT AND $current_memory_limit > 0))
{
    @ini_set('memory_limit', MIN_LIMIT);
}
...
class XML_Parser
{
    ...
    /**
      * Kludge to include the top level element since this parser was written to
      * not return it and now all of the XML functions assume it to not be there
      *
      * @var	boolean
      */
    ...
    /**
      * Error code from XML object prior to releases of resources. This needs to
      * be done to avoid a segfault in PHP 4. See Bug#24425
      *
      * @var integer
      */
    var $error_code = 0;
    ...
    /**
      * Error line number from XML object prior to releases of resources. This needs
      * to be done to avoid a segfault in PHP 4. See Bug#24425
      *
      * @var integer
      */
    var $error_line = 0;
    ...
    /**
      ...
      * @see XML_Parser::set_target_encoding()
      *
      * @var string
      */
    var $target_encoding;
    ...
    function &parse($encoding = 'ISO-8859-1', $emptydata = true)
    {
        ...
        xml_set_element_handler       ( $this -> xml_parser, array(&$this, 'handle_element_start'),
                                                             array(&$this, 'handle_element_end')
                                      );
    ...
    function legacy_parse_xml()
    {
        ...
        if ( preg_match('#(<?xml.*encoding=[\'"])(.*?)([\'"].*?>)#m', $this -> xmldata, $match) )
        {
            ...
            if ($in_encoding != 'UTF-8' OR strtoupper(CHARSET != 'UTF-8'))
            {
        ...
        $target_encoding = (strtolower(CHARSET) == 'iso-8859-1' ? 'WINDOWS-1252' : CHARSET);
        $xml_encoding    = (($in_encoding != 'UTF-8' OR strtoupper(CHARSET) != 'UTF-8') ? 'ISO-8859-1' : 'UTF-8');
        $iconv_passed    = false;
    ...
    function handle_element_start(&$parser, $name, $attribs)
    {
        ...
        array_unshift($this -> stack, array('name'      => $name,
                                            'attribs'   => $attribs,
