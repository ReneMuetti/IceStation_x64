...
define('TYPE_FLOAT',            14); // force float
...
class Input_Cleaner
{
	...
	*/
	function __construct(&$registry)
	{
		...
		if ( isset($_SERVER['REQUEST_METHOD']) AND isset($_SERVER['HTTP_X_REQUESTED_WITH']) )
		{
		...
		switch ($type)
		{
			...
			case TYPE_UNIXTIME:
			{
				if (is_array($data))
				{
					...
					{
						$data = mymktime($data['hour'], $data['minute'], $data['second'], $data['month'], $data['day'], $data['year']);
					}
					else
					{
						$data = 0;
	...
	*/
	function fetch_scriptpath()
	{
		...
		{
			if ( !empty($_SERVER['REQUEST_URI']) OR !empty($_ENV['REQUEST_URI']) )
			{
			...
			else
			{
				...
				else if ( !empty($_SERVER['REDIRECT_URL']) OR !empty($_ENV['REDIRECT_URL']) )
				{
				...
				if ( !empty($_SERVER['QUERY_STRING']) OR !empty($_ENV['QUERY_STRING']) )
				{
